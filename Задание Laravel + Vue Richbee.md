...
- `Role` - роли пользователей (admin, manager, dispatcher, master, otk)
- `Company` - контрагенты (заказчики)
- `Product` - продукция (целевые изделия и материалы)
- `Order` - заказы от контрагентов (один продукт на заказ)
- `ProductionTask` - задание в производство
- `TaskComponent` - компоненты для задания (материалы в штуках)
...
- Методы attach(), detach(), sync(), syncWithoutDetaching()
- Дополнительные поля в pivot таблице
- Получение данных pivot через ->withPivot()
...
- Eager Loading: `with()` для предзагрузки связей
- Проблема N+1 запросов
- Методы `withCount()`, `has()`, `whereHas()`
- `load()` для ленивой загрузки
- `cursor()` и `chunk()` для больших выборок
...
- Примените фильтрацию через связанные таблицы
...
- Методы restore(), forceDelete()
- Работа с удаленными записями: withTrashed(), onlyTrashed()
- Каскадное мягкое удаление связанных записей
...
- Фильтрация с учетом удаленных данных
...
- Метод `rules()` для правил валидации
- Метод `authorize()` для проверки прав
- Кастомные сообщения ошибок
- Подготовка данных через `prepareForValidation()`
...
### Теория
Изучите паттерн DTO:
- Назначение DTO для передачи данных между слоями
- Отличие от моделей Eloquent
